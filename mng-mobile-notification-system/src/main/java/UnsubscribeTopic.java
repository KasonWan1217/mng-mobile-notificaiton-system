...
import static util.DBEnumValue.ArnType.Platform;
import static util.ErrorMessageUtil.ErrorMessage.*;

public class UnsubscribeTopic implements RequestHandler<APIGatewayProxyRequestEvent, APIGatewayProxyResponseEvent> {
    ...
    public APIGatewayProxyResponseEvent handleRequest(final APIGatewayProxyRequestEvent input, final Context context) {
        ...
        if (input != null) {
            ...
            //Get the platform list with subscribed topic from Sns Account
            ...
            if (list_subscribed_record.size() < 1)
                return response.withStatusCode(200).withBody(ErrorMessageUtil.getErrorResponseMessage(Topic_subscription_List_Null_Error).convertToJsonString());

            List<Subscription> list_new_record = new ArrayList<>();
            //remove the subscription in account
            snsAccount.getSubscriptions().removeIf(item -> request.getChannel_name().equals(item.getChannel_name()));
            for (Subscription subscription : list_subscribed_record) {
                fs_all.add(SNSNotificationService.unsubscribe(subscription.getArn()));
                if (!fs_all.get(fs_all.size() - 1).isStatus()) {
                    //Unsubscribe fails, Start to Subscribe the topic
                    for (Subscription temp : platform_list) {
                        fs_all.add(SNSNotificationService.subscribe(temp.getArn(), CommonUtil.getSnsTopicArn(request.getChannel_name())));
                        if (fs_all.get(fs_all.size() - 1).isStatus()) {
                            String subscriptionArn = fs_all.get(fs_all.size() - 1).getResponse().get("subscriptionArn").toString();
                            list_new_record.add(new Subscription(request.getChannel_name(), subscriptionArn, DBEnumValue.ArnType.Topic.toString(), subscription.getChannel_name(), CommonUtil.getCurrentTime()));
                        }
                    }
                    ...
                    return response.withStatusCode(200).withBody(new ResponseMessage(Sns_Unsubscribe_Error.getCode(), list_errorMessage).convertToJsonString());
                }
            }
